...
from core.server.post_processor.base_post_processor import BasePostProcessor
...
class HeteroRunner(FederatedRunner):
    ...
    def _setup(self):
        ...
        # 构建后处理器
        post_processor_conf = server_conf.get('post_processor', {})
        post_processor = BasePostProcessor(config=post_processor_conf)
        
        # 使用标准 BaseClient 构建 dataloader
        self.client_class = BaseClient

        # 构建测试集 DataLoader（Runner 现已管理 test_loader）

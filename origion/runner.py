...
from core.utils.configs import DataConfig, TrainingConfig, ClientConfig
...
import core.server.post_processor
import core.attack
import algorithms
...
# Runner 默认常量 (逐步迁移到 configs.py，保留部分兼容)
DEFAULT_SEED = 42
...
class FederatedRunner:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Logger 在此仅完成参数配置，不触发任何 IO / 网络请求
        self.logger = Logger.from_config(config)
        self.logger.start()
    ...
    def _setup_data_pipeline(self):
        ...
        self.logger.info(f"Data setup complete. Clients: {self.data_config.num_clients}")
    ...
    def _setup_server_and_algorithm(self):
        ...
        # 构建测试集 DataLoader（原来在 algorithm builder 中，现在移到 Runner）
        from torch.utils.data import Subset, DataLoader
        server_dataset = Subset(self.server_dataset_store, self.server_test_task.indices)
        batch_size = self.config.get('client', {}).get('batch_size', 64)
        self.test_loader = DataLoader(server_dataset, batch_size=batch_size, shuffle=False, num_workers=0)
    ...
    def run(self):
        ...
        try:
            ...
            # 保证 Logger 在异常崩溃时也能正常 close（WandB run 状态正确同步）
            self.logger.close()
    ...
    def _eval_clean_global(self, round_idx):
        ...
        clean_metrics = self._eval_model(metrics=clean_metric_objs)
    ...
    def _eval_attack_groups(self, round_idx):
        ...
        for group_name, group_config in attackers_conf.items():
            ...
            attack_metrics = self._eval_model(
    ...
    def _eval_model(self, metrics: List[Callable], dataloader=None) -> Dict[str, float]:
        """
        在指定数据集上评估全局模型。
        原 BaseServer.eval()，迁移到 Runner。
        """
    ...
    def _aggregate_client_metrics(self, client_payloads):
        """
        聚合客户端返回的 metrics (如 train_loss)。
        通常是按样本数加权平均。
        原 BaseServer.aggregate_metrics()，迁移到 Runner。
        """
